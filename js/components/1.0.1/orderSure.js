...
define([
	  ...
	  InfoChannel.getDataByAjax(url, param, function (data) {
			...
			if (data.flag) {
				  ...
				  if(allPrice.orderPrice-79>0&&allPrice.shippingFee==8.00){
						allPrice.always=true;
				  }

				  allPrice.routineFee=parseFloat((0.02*allPrice.orderPrice).toFixed(2));
				  $("[data-attach-point=routineFee] span").html(allPrice.routineFee)
				  /*****优惠券*******/
	  ...
	  var addGiftCard = function () {
			...
			if (utilTool.checkGiftCard(paramCard.couponCode)){
                 InfoChannel.getDataByAjax(url,paramCard,function(data){
     ...
     var invoiceN=0;
	  ...
	  var payOnline = function () {//在线支付
            ...
            else if(tMoney-79<0&&allPrice.shippingFee==0){
				  allPrice.shippingFee=8;
			}else if(tMoney-79>0&&allPrice.shippingFee==8){
	  ...
	  var payDownline = function () {//货到付款
			$("#integral").hide();
			allPrice.totalMoney=allPrice.orderPrice-allPrice.voucher-allPrice.giftCartPrice+8+allPrice.routineFee;
			$("#shipping").html(8.00);
			$("#totalMoney").html(allPrice.totalMoney.toFixed(2));
